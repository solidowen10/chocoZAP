...
const SHORTLIST_SHEET_PLATFORM_COLUMN_COUNT = 15;
const SHORTLIST_SHEET_TOTAL_COLUMN_COUNT = 23;
...
function isBasementB1Floor(floorText) {
  const value = clean(floorText).toUpperCase();
  if (!value) return false;
  return /(^|[\\s,\\/])B1F?(?=$|[\\s,\\/])/.test(value);
}
...
function mergeImportedListing(existing = {}, incoming = {}, now = new Date().toISOString()) {
  ...
  for (const field of HUMAN_FIELDS) {
    if (field === 'status') {
      const existingStatus = oldHuman.status || DEFAULT_STATUS;
      merged.status =
        existingStatus === DEFAULT_STATUS && isBasementB1Floor(crawler.floor_text)
          ? 'rejected'
          : existingStatus;
      continue;
    }
  ...
  if (
    ...
    isBasementB1Floor(crawler.floor_text) &&
    isBlank(merged.rejected_reason)
  ) {
    merged.rejected_reason = 'B1 地下樓層';
  }
...
function scoreListing(listing, rawConfig = DEFAULT_SCORING_CONFIG) {
  ...
  // Hard reject: listings whose main floor is underground only.
  // Examples rejected: B1, B1F, 地下1樓, 地下一樓, 地下室.
  // Mixed floors such as 1F/B1 are NOT rejected here.
  const normalizedFloor = normalizeFloor(x.floor_text);
  const mainFloor = normalizedFloor.split("/")[0];
  const undergroundOnly = /^(?:B[1-9][0-9]*F?|地下(?:[一二三四五六七八九十0-9]+樓|室))$/.test(mainFloor);
  const secondFloorOnly = /^2F?$/.test(mainFloor);

  if (undergroundOnly || secondFloorOnly) {
    const floorReason = undergroundOnly ? "地下樓層" : "2F 樓層";
    return {
      score: 0,
...
function shortlistSheetRowIdentity(row = []) {
  const source = clean(row[0]);
  const sourceListingId = clean(row[1]);
  if (!source || !sourceListingId) return null;
  return `${source}:${sourceListingId}`;
}

function rentPerPingForListing(listing = {}) {
  const x = normalizeListing(listing);
  if (x.rent_twd == null || x.listed_area_ping == null || x.listed_area_ping <= 0) return '';
  return Math.round(x.rent_twd / x.listed_area_ping);
}

function shortlistAddedAt(listing = {}, now = new Date().toISOString()) {
  return (
    nullableString(listing.shortlisted_at) ||
    nullableString(listing.shortlistedAt) ||
    nullableString(listing.reviewed_at) ||
    nullableString(listing.reviewedAt) ||
    now
  );
}

function shortlistPlatformRow(listing = {}, options = {}) {
  const x = normalizeListing(listing);
  const score = listing.score == null
    ? scoreListing(x, options.config || DEFAULT_SCORING_CONFIG).score
    : number(listing.score);

  const nearbyIsMrt = x.nearby_place_type === 'mrt';
  const rentPerPingTwd = rentPerPingForListing(x);
  const twdJpyRate = number(options.twdJpyRate);

  const rentPerPingJpy =
    rentPerPingTwd !== '' && twdJpyRate != null
      ? Math.round(Number(rentPerPingTwd) * twdJpyRate)
      : '';

  return [
    sheetCell(x.source),                                      // A
    sheetCell(x.source_listing_id),                           // B
    sheetCell(x.title),                                       // C
    sheetCell(x.url),                                         // D
    sheetCell(x.city),                                        // E
    sheetCell(x.district),                                    // F
    sheetCell(x.address),                                     // G
    sheetCell(x.rent_twd),                                    // H
    sheetCell(x.listed_area_ping),                            // I
    sheetCell(rentPerPingTwd),                                // J
    sheetCell(rentPerPingJpy),                                // K
    sheetCell(nearbyIsMrt ? x.nearby_place : x.mrt_station),  // L
    sheetCell(nearbyIsMrt ? x.nearby_distance_m : ''),        // M
    sheetCell(score),                                         // N
    sheetCell(options.addedAt || shortlistAddedAt(listing, options.now)), // O
  ];
}

function buildShortlistSheetSyncPlan(listings = [], existingValues = [], options = {}) {
  const now = options.now || new Date().toISOString();
  const values = Array.isArray(existingValues) ? existingValues : [];
  const existingByIdentity = new Map();

  values.slice(1).forEach((row, index) => {
    const identity = shortlistSheetRowIdentity(row);
  ...
  const shortlistedListings = (listings || []).filter((listing) =>
  ...
  for (const listing of shortlistedListings) {
    const normalized = normalizeListing(listing);
    const identity = listingIdentity(normalized);
    ...
    // Added-at moved from N to O.
    const existingAddedAt =
      existing && !isBlank(existing.row[14])
        ? existing.row[14]
        : '';

    const platformRow = shortlistPlatformRow(listing, {
      now,
      addedAt: existingAddedAt || shortlistAddedAt(listing, now),
      config: options.config,
      twdJpyRate: options.twdJpyRate,
    });

    const fxValues = [
      sheetCell(options.twdJpyRate),
      sheetCell(options.fxUpdatedAt || now),
    ];
    ...
    if (existing) {
      updates.push({
        ...
        values: platformRow.slice(0, SHORTLIST_SHEET_PLATFORM_COLUMN_COUNT),
        fxValues,
      });
    } else {
      appendRows.push([
        ...platformRow.slice(0, SHORTLIST_SHEET_PLATFORM_COLUMN_COUNT),

        // P:U broker-managed fields.
        ...Array(6).fill(''),
  ...
  return {
    shortlisted: shortlistedListings.length,
    total: updates.length + appendRows.length,
