...
const SHORTLIST_SHEET_PLATFORM_COLUMN_COUNT = 14;
const SHORTLIST_SHEET_TOTAL_COLUMN_COUNT = 20;
...
function mergeImportedListing(existing = {}, incoming = {}, now = new Date().toISOString()) {
  ...
  for (const field of HUMAN_FIELDS) {
    if (field === 'status') {
      merged.status = oldHuman.status || DEFAULT_STATUS;
      continue;
    }
...
function shortlistPlatformRow(listing = {}, options = {}) {
  ...
  return [
    sheetCell(x.source),
    sheetCell(x.source_listing_id),
    sheetCell(x.title),
    sheetCell(x.url),
    sheetCell(x.city),
    sheetCell(x.district),
    sheetCell(x.address),
    sheetCell(x.rent_twd),
    sheetCell(x.listed_area_ping),
    sheetCell(rentPerPingForListing(x)),
    sheetCell(nearbyIsMrt ? x.nearby_place : x.mrt_station),
    sheetCell(nearbyIsMrt ? x.nearby_distance_m : ''),
    sheetCell(score),
    sheetCell(options.addedAt || shortlistAddedAt(listing, options.now)),
  ];
}
...
function buildShortlistSheetSyncPlan(listings = [], existingValues = [], options = {}) {
  ...
  for (const listing of shortlistedListings) {
    ...
    const existingAddedAt = existing && !isBlank(existing.row[13]) ? existing.row[13] : '';
    ...
    if (existing) {
      ...
      appendRows.push([
        ...
        ...Array(SHORTLIST_SHEET_TOTAL_COLUMN_COUNT - SHORTLIST_SHEET_PLATFORM_COLUMN_COUNT).fill(''),
      ]);
    }
  }
