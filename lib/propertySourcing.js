...
const DEFAULT_SCORING_CONFIG = {
  ...
  floor: {
    ground_floor_points: 2.5,
    ground_plus_second_points: 2,
    ground_plus_basement_points: 1.8,
    second_floor_points: 1,
  },
};
...
function normalizeScoringConfig(config) {
  ...
  return {
    version: 1,
    recommendation_threshold: configNumber(merged.recommendation_threshold, DEFAULT_SCORING_CONFIG.recommendation_threshold, {
      min: 0,
      max: SCORING_TOTAL_MAX,
    }),
    city_location: {
      preferred_cities: configList(
        merged.city_location?.preferred_cities,
        DEFAULT_SCORING_CONFIG.city_location.preferred_cities,
      ),
      preferred_districts: configList(
        merged.city_location?.preferred_districts,
        DEFAULT_SCORING_CONFIG.city_location.preferred_districts,
      ),
      city_points: configPoint(
        merged.city_location?.city_points,
        DEFAULT_SCORING_CONFIG.city_location.city_points,
        SCORING_MAX_POINTS.city_location,
      ),
      district_bonus_points: configPoint(
        merged.city_location?.district_bonus_points,
        DEFAULT_SCORING_CONFIG.city_location.district_bonus_points,
        SCORING_MAX_POINTS.city_location,
      ),
    },
    area: {
      good_min_ping: configNumber(merged.area?.good_min_ping, DEFAULT_SCORING_CONFIG.area.good_min_ping, { min: 0 }),
      good_max_ping: configNumber(merged.area?.good_max_ping, DEFAULT_SCORING_CONFIG.area.good_max_ping, { min: 0 }),
      ok_min_ping: configNumber(merged.area?.ok_min_ping, DEFAULT_SCORING_CONFIG.area.ok_min_ping, { min: 0 }),
      good_points: configPoint(merged.area?.good_points, DEFAULT_SCORING_CONFIG.area.good_points, SCORING_MAX_POINTS.area),
      ok_points: configPoint(merged.area?.ok_points, DEFAULT_SCORING_CONFIG.area.ok_points, SCORING_MAX_POINTS.area),
      low_points: configPoint(merged.area?.low_points, DEFAULT_SCORING_CONFIG.area.low_points, SCORING_MAX_POINTS.area),
    },
    rent: {
      good_max_twd: configNumber(merged.rent?.good_max_twd, DEFAULT_SCORING_CONFIG.rent.good_max_twd, { min: 0 }),
      ok_max_twd: configNumber(merged.rent?.ok_max_twd, DEFAULT_SCORING_CONFIG.rent.ok_max_twd, { min: 0 }),
      good_points: configPoint(merged.rent?.good_points, DEFAULT_SCORING_CONFIG.rent.good_points, SCORING_MAX_POINTS.rent),
      ok_points: configPoint(merged.rent?.ok_points, DEFAULT_SCORING_CONFIG.rent.ok_points, SCORING_MAX_POINTS.rent),
      high_points: configPoint(merged.rent?.high_points, DEFAULT_SCORING_CONFIG.rent.high_points, SCORING_MAX_POINTS.rent),
    },
    floor: {
      ground_floor_points: configPoint(
        merged.floor?.ground_floor_points,
        DEFAULT_SCORING_CONFIG.floor.ground_floor_points,
        SCORING_MAX_POINTS.floor,
      ),
      ground_plus_second_points: configPoint(
        merged.floor?.ground_plus_second_points,
        DEFAULT_SCORING_CONFIG.floor.ground_plus_second_points,
        SCORING_MAX_POINTS.floor,
      ),
      ground_plus_basement_points: configPoint(
        merged.floor?.ground_plus_basement_points,
        DEFAULT_SCORING_CONFIG.floor.ground_plus_basement_points,
        SCORING_MAX_POINTS.floor,
      ),
      second_floor_points: configPoint(
        merged.floor?.second_floor_points,
        DEFAULT_SCORING_CONFIG.floor.second_floor_points,
        SCORING_MAX_POINTS.floor,
      ),
    },
  };
...
function normalizeCrawlerListing(input = {}) {
  return {
    ...
    mrt_distance_m: integer(input.mrt_distance_m),
...
function mergeImportedListing(existing = {}, incoming = {}, now = new Date().toISOString()) {
  ...
  for (const field of HUMAN_FIELDS) {
    ...
    if (field === 'mrt_station') {
      merged.mrt_station = !isBlank(existing.mrt_station)
        ? oldHuman.mrt_station
        : !isBlank(incoming.mrt_station)
          ? incomingHuman.mrt_station
          : oldHuman.mrt_station;
      continue;
    }
...
function listingFromSheetRow(row = {}) {
  return normalizeListing({
    ...
    mrt_distance_m: rowValue(row, ['mrt_distance_m', '捷運距離M', '捷運距離', '距離公尺']),
...
function scoreListing(listing, rawConfig = DEFAULT_SCORING_CONFIG) {
  ...
  const cityPoints = cityHit ? earnedPoints(config.city_location.city_points, SCORING_MAX_POINTS.city_location) : 0;
  const districtPoints = districtHit
    ? earnedPoints(config.city_location.district_bonus_points, SCORING_MAX_POINTS.city_location)
    : 0;
  const cityLocationPoints = earnedPoints(cityPoints + districtPoints, SCORING_MAX_POINTS.city_location);
  score += cityLocationPoints;
  if (!x.city) missing.push('城市');
  if (!x.district) missing.push('行政區');
  addBreakdown(
    breakdown,
    'city_location',
    '城市 / 區域',
    ...
    SCORING_MAX_POINTS.city_location,
    cityHit
      ? `${x.city || ''}${x.district ? ` ${x.district}` : ''}${districtHit ? '' : '（非優先行政區）'}`
      : '非優先城市',
    !x.city || !x.district ? 'missing' : 'known',
  );

  const listedArea = x.listed_area_ping;
  if (listedArea == null) {
    missing.push('總坪數');
    addBreakdown(breakdown, 'area', '總坪數', 0, SCORING_MAX_POINTS.area, '待確認', 'missing');
  } else if (listedArea >= config.area.good_min_ping && listedArea <= config.area.good_max_ping) {
    const points = earnedPoints(config.area.good_points, SCORING_MAX_POINTS.area);
    score += points;
    addBreakdown(breakdown, 'area', '總坪數', points, SCORING_MAX_POINTS.area, `${listedArea} 坪符合目標`);
  } else if (listedArea >= config.area.ok_min_ping) {
    const points = earnedPoints(config.area.ok_points, SCORING_MAX_POINTS.area);
    score += points;
    risks.push(`總坪數 ${listedArea} 坪可接受但非最佳`);
    addBreakdown(breakdown, 'area', '總坪數', points, SCORING_MAX_POINTS.area, `${listedArea} 坪可接受但非最佳`);
  } else {
    const points = earnedPoints(config.area.low_points, SCORING_MAX_POINTS.area);
    score += points;
    risks.push(`總坪數 ${listedArea} 坪偏小`);
    addBreakdown(breakdown, 'area', '總坪數', points, SCORING_MAX_POINTS.area, `${listedArea} 坪偏小`);
  }

  if (x.rent_twd == null) {
    missing.push('租金');
    addBreakdown(breakdown, 'rent', '租金', 0, SCORING_MAX_POINTS.rent, '待確認', 'missing');
  } else if (x.rent_twd <= config.rent.good_max_twd) {
    const points = earnedPoints(config.rent.good_points, SCORING_MAX_POINTS.rent);
    score += points;
    addBreakdown(breakdown, 'rent', '租金', points, SCORING_MAX_POINTS.rent, `NT$${x.rent_twd.toLocaleString()}/月`);
  } else if (x.rent_twd <= config.rent.ok_max_twd) {
    const points = earnedPoints(config.rent.ok_points, SCORING_MAX_POINTS.rent);
    score += points;
    addBreakdown(breakdown, 'rent', '租金', points, SCORING_MAX_POINTS.rent, `NT$${x.rent_twd.toLocaleString()}/月偏高`);
  } else {
    const points = earnedPoints(config.rent.high_points, SCORING_MAX_POINTS.rent);
    score += points;
    risks.push(`租金 NT$${x.rent_twd.toLocaleString()}/月偏高`);
    addBreakdown(breakdown, 'rent', '租金', points, SCORING_MAX_POINTS.rent, `NT$${x.rent_twd.toLocaleString()}/月過高`);
  }

  const floor = normalizeFloor(x.floor_text);
  if (!floor) {
    missing.push('樓層');
    addBreakdown(breakdown, 'floor', '樓層', 0, SCORING_MAX_POINTS.floor, '待確認', 'missing');
  } else if (/1F[+~]2F|1F.*2F/.test(floor)) {
    const points = earnedPoints(config.floor.ground_plus_second_points, SCORING_MAX_POINTS.floor);
    score += points;
    addBreakdown(breakdown, 'floor', '樓層', points, SCORING_MAX_POINTS.floor, floor);
  } else if (/1F[+~]B1|1F.*B1|B1.*1F/.test(floor)) {
    const points = earnedPoints(config.floor.ground_plus_basement_points, SCORING_MAX_POINTS.floor);
    score += points;
    addBreakdown(breakdown, 'floor', '樓層', points, SCORING_MAX_POINTS.floor, floor);
  } else if (/^1F(?:\/|$)/.test(floor)) {
    const points = earnedPoints(config.floor.ground_floor_points, SCORING_MAX_POINTS.floor);
    score += points;
    addBreakdown(breakdown, 'floor', '樓層', points, SCORING_MAX_POINTS.floor, floor);
  } else if (/^2F(?:\/|$)|[^0-9]2F(?:\/|$)/.test(floor)) {
    const points = earnedPoints(config.floor.second_floor_points, SCORING_MAX_POINTS.floor);
    score += points;
    risks.push('2F 物件需確認動線與招牌露出');
    addBreakdown(breakdown, 'floor', '樓層', points, SCORING_MAX_POINTS.floor, floor);
  } else {
    risks.push(`樓層 ${floor} 需人工確認`);
    addBreakdown(breakdown, 'floor', '樓層', 0, SCORING_MAX_POINTS.floor, floor);
  }

  score = roundTenth(score);

  let recommendation = 'needs_research';
  if (x.status === 'rejected') recommendation = 'reject';
...
function listingToLocationReviewItem(listing, config) {
  ...
  return {
    ...
    finder: '591 Radar',
    floor: x.floor_text || '',
    mrt: x.mrt_minutes == null ? (x.mrt_distance_m == null ? '' : `${x.mrt_distance_m} 公尺`) : `${x.mrt_minutes} 分鐘`,
