...
function buildDigestText(org, locations) {
  const lines = [`📋 ${org.name} — Progress Update`, ''];
  const byStage = {};
  locations.forEach((l) => {
    const key = l.stage_name || 'No stage';
    (byStage[key] = byStage[key] || []).push(l);
  });
  Object.entries(byStage).forEach(([stage, locs]) => {
    lines.push(`■ ${stage} (${locs.length})`);
    locs.forEach((l) => {
      const flag = l.blocked_count > 0 ? ` ⚠️${l.blocked_count} blocked` : '';
      lines.push(`  • ${l.name}: ${l.progress_pct}%${flag}`);
    });
    lines.push('');
  });
  lines.push(`View full workspace: ${process.env.APP_URL || ''}`);
